...
void CPlayerBehavior_Move::Action(CPlayer* player)
{
	...
	if (player->GetOnStand())
	{
		...
		else if (CManager::GetInstance()->GetMouse()->GetTrigger(CManager::GetInstance()->GetMouse()->MOUSEBUTTON_LEFT) || CManager::GetInstance()->GetJoypad()->GetTrigger(CInputJoypad::JOYKEY_X))
		{
...
void CPlayerBehavior_NormalAttack002::Behavior(CPlayer* player)
{
	if (!m_bChargeEnd)
	{
		...
		//攻撃ボタンを離したら攻撃
		if (CManager::GetInstance()->GetMouse()->GetRerease(CManager::GetInstance()->GetMouse()->MOUSEBUTTON_LEFT) || CManager::GetInstance()->GetJoypad()->GetRerease(CInputJoypad::JOYKEY_X) || m_fChargeRate >= MAX_RATE)
		{
			//キャンセル時間後に離したらチャージ
			if (m_nCancelCount >= ACCEPT_CANCELTIME)
			{
